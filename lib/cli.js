...
var argv = require('optimist').
    ...
    describe('chromeArgs', 'Comma-seperated list of Chrome-specific arguments').
    describe('phantomArgs', 'Comma-seperated list of Command-line arguments to pass to PhantomJS').
    ...
    alias('chromeArgs', 'capabilities.chromeArgs').
    alias('phantomArgs', 'capabilities.phantomArgs').
...
if(argv.chromeArgs) {
    argv.capabilities.chromeOptions = {};
    argv.capabilities.chromeOptions.args = argv.chromeArgs.split(',');
}

if(argv.phantomArgs) {
    var phantomArgs = argv.phantomArgs.split(',');
    phantomArgs.forEach(function(arg, index, arr) {
        arr[index] = '--' + arr[index];
    });

    argv.capabilities['phantomjs.cli.args'] = phantomArgs;
}
